...
check_information <- function(jws, idx_sap = NULL, idx_sai = NULL) {
    ...
    if (!is.null(idx_sap) && max(idx_sap) > rjd3workspace::.jws_sap_count(jws)) {
        stop("The SAP n°", max(idx_sap), "doesn't exist")
    } else if (is.null(idx_sap)) {
        idx_sap <- seq_len(rjd3workspace::.jws_sap_count(jws))
    }
...
spreadsheet_update_path <- function(jws, new_path, idx_sap = NULL, idx_sai = NULL) {
    ...
    for (id_sap in idx_sap) {
        jsap <- rjd3workspace::.jws_sap(jws, idx = id_sap)

        idx_sai_tmp <- idx_sai
        if (is.null(idx_sai)) {
            idx_sai_tmp <- seq_len(rjd3workspace::.jsap_sa_count(jsap))
        }

        for (id_sai in idx_sai_tmp) {
            jsai <- rjd3workspace::.jsap_sa(jsap, idx = id_sai)

            old_jd3_ts <- rjd3workspace::get_ts(jsai)
            ...
            rjd3workspace::set_ts(jsap = jsap, idx = id_sai, y = new_jd3_ts)
        }
    }
